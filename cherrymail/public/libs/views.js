...
const { REACT_PATH, EMAIL_SERVICES } = require('../constants');
...
const addView = (browserWindow, workspace) => {
  ...
  if (workspace.id === 'home') {
    ...
    view.webContents.loadURL(REACT_PATH);
  } else {
    view = new BrowserView({
      webPreferences: {
    ...
    view.webContents.on('new-window', (e, nextUrl) => {
      const curDomain = extractDomain(EMAIL_SERVICES[workspace.serviceId].url);
    ...
    // https://github.com/quanglam2807/cherrymail/issues/182
    ...
    view.webContents.loadURL((rememberLastPageVisited && workspace.lastUrl)
      || EMAIL_SERVICES[workspace.serviceId].url);
  }
