...
module.exports = React.createClass({
	...
	getInitialState () {
		return {
			selectedMode: this.props.selectedMode || 'category-most-installed'
		}
	},
	...
	handleModeChange (newMode) {
		...
		WebAppActions.fetch({ type: newMode, data: this.props.category.id });

		this.setState({ selectedMode })
	},

	componentDidMount() {
		if (this.state.webAppStore.get('type') != this.state.selectedMode || this.state.webAppStore.get('data') != this.props.category.id)
			WebAppActions.fetch({ type: this.state.selectedMode, data: this.props.category.id });
	},
	...
	render () {
		...
		if (status == 'failed') {
			...
			results = (
				<UI.GroupBody>
					{<UI.ListHeader sticky>{this.state.selectedMode == 'category-most-installed' ? 'Most Installed Apps' : 'New Apps'}</UI.ListHeader>}
					{
						webApps.map((webApp, i) => {
							return <WebAppItem key={'webApp' + i} webApp={webApp} prevProps={{ selectedMode: this.state.selectedMode, category: this.props.category, prevView: this.props.prevView }} prevView='category'/>
						})
					}
				</UI.GroupBody>
			)
        ...
        <div className="Alert-Container" style={{ display: this.state.webAppStore.get('status') == 'loading' ? '' : 'none' }}>
          <UI.Alertbar type="warning" pulse>Loading...</UI.Alertbar>
				...
				<UI.SegmentedControl value={this.state.selectedMode} onChange={this.handleModeChange} hasGutter equalWidthSegments options={[
					{ label: 'Most Installed', value: 'category-most-installed' },
					{ label: 'New', value: 'category-new' }
				]} />
				{results}
