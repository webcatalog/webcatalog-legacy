...
const getElectronPath = () => {
  if (process.platform === 'darwin') {
    return path.resolve(__dirname, '..', '..', 'dist', 'mac', 'Lyn.app', 'Contents', 'MacOS', 'Lyn');
  }

  if (process.platform === 'linux') {
    return path.resolve(__dirname, '..', '..', 'dist', 'linux-unpacked', 'lyn');
  }

  return path.resolve(__dirname, '..', '..', 'dist', 'win-unpacked', 'Lyn.exe');
};
